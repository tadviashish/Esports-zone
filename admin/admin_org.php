...
<body>
    ...
    <div class="container">
        ...
        <div class="profile2">
            ...
            <form action="delete_multiple_org.php" method="POST">
            <input type="submit" name="delete_btn" value="Delete Selected" class="delete-button">
                <ul class="athlete-list" id="athlete-results">
                    <?php
                    $sql = "SELECT * FROM org_data";
                    $rows = mysqli_query($conn, $sql);
                    foreach ($rows as $row) {
                    ?>
                        <li class="athlete-item">
                            <input type="checkbox" name="delete_ids[]" value="<?php echo $row['orgid']; ?>">
                            <span class="athlete-name"><?php echo $row['orgname']; ?></span>
                            <a href="admin_view_org.php?id=<?php echo $row['orgid']; ?>">View Details</a>
                            <a href="deleteorg.php?id=<?php echo $row['orgid']; ?>">Delete</a>
                        </li>
                    <?php } ?>
                </ul>
            </form>
            ...
            <div class="pagination" id="pagination">
                <?php
                $total_sql = "SELECT COUNT(*) FROM org_data";
    ...
    <script>
        document.getElementById("search").addEventListener("keyup", function() {
            ...
            xhr.open("GET", "search_org.php?q=" + encodeURIComponent(query), true);
            xhr.onload = function() {
                if (this.status == 200) {
